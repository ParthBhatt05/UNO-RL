...
class UnoEnvironment:
        ...
        if self.legal_move(action):
            if self.to_draw > 0:
                if played_card is None:
                    # player draw cards from previous draw 2 or draw 4 card(s)
                    player.draw_cards(self.to_draw)
                    self.to_draw = 0
                    player_status = 0
                elif self.top_card[1] == 11 and played_card[1] == 11:
                    # player adds draw 2 card to existing
                    self.to_draw += 2
                    player_status = 1
                elif self.top_card[1] == 14 and played_card[1] == 14:
                    # player adds draw 4 card to existing
                    self.to_draw += 4
                    player_status = 1
    ...
    def get_state(self):
        # all cards excluding wild card and draw 4 card
        ...
        # one hot vector indicating colour of draw 4 card on top of the stack
        ...
        # how many cards the next player has to draw (due to draw 2 or draw 4 cards)
    ...
    def legal_move(self, action, player=None):
        ...
        if player.cards[action] == 0:
            return False
        # check if player has to draw cards after draw 2 or also plays draw 2
        if self.to_draw > 0 and self.top_card[1] == 11 and card[1] != 11:
            return False
        # check if player has to draw cards after draw 4 or also plays draw 4
        if self.to_draw > 0 and self.top_card[1] == 14 and card[1] != 14:
            return False
        # if conditions so far were fulfilled it is always legal to play wild or draw 4 cards
...
class UnoPlayer:
    ...
    def play_card(self, card_index, colour=None):
        ...
        # set the colour of the played card if the colour was provided (for wild and draw 4 cards)
